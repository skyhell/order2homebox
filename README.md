...
- ✏️ **Review before create** — every scraped item is editable; locations are read
  ...
  last used is pre-selected for every item of the next order, and picking one
  (or creating one) on any card applies it to all the other cards — an order
...
- 🏷️ **QR labels** — DK-22211 (29 mm endless), exactly 306 px wide, two identical
  ...
  QR label without changing the default. QR content is Homebox's native
  `…/a/{asset_id}` deep link.
