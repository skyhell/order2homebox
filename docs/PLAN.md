...
2. Edit-Seite: Kopfzeile mit Bestellnr., Datum und **Shop als editierbarem Dropdown** (v0.14.0 — vorher ein verstecktes Feld, daher landete „manuell erfassen" immer bei Amazon und war dort nicht korrigierbar; „Sonstiges …" klappt ein Textfeld für einen Shop ohne eigenen Scraper auf, der Wert reist unverändert in `purchaseFrom`; die Kopfzeile ist seit v0.14.1 ein 2-Spalten-Grid — der Shop hat die erste Zeile für sich, weil er im „Sonstiges"-Zustand zwei Bedienelemente trägt und in einer Drittelspalte auf seinen Pfeil zusammenschrumpfte, Bestellnr. und Datum teilen die zweite; die Zeile „Bestellung … bei …" darüber folgt dem Feld per JS, der Satz selbst bleibt in der Locale-Datei). Formular je Artikel (Name, Beschreibung, Menge, Preis, Bestellnr.), **Lagerort-Dropdown live aus Homebox**, „+ Neuer Lagerort“ (Inline-Feld, htmx-`POST /locations` → in Homebox anlegen, vorauswählen, Formulardaten bleiben erhalten; ein getippter, aber nie über diesen Button bestätigter Name wird jetzt beim Anlegen selbst noch aufgelöst statt verworfen — v0.13.6), „Auf alle Karten übernehmen“ (v0.8.0, nur ab der zweiten Karte sichtbar), Homebox-Labels, Checkboxen „drucken“ und „Asset-ID aufs Etikett“ (je Artikel).
2a. `GET /manual` (v0.15.0) — **eigene Seite für Artikel ohne Bestellabruf**, eigener Nav-Eintrag direkt nach „Neue Bestellung“ (vorher ein Link unter dem Abruf-Formular). Derselbe Aufbau wie die Edit-Seite, nur ohne Shop-Dropdown: hier wurde nichts abgerufen, also wird der Shop getippt (freies Textfeld, geht unverändert in `purchaseFrom`). Die Seite hat einen **eigenen Entwurf** (`data/manual.json` neben `data/draft.json`; welcher gemeint ist, sagt das versteckte Feld `mode` im Formular — `_store()` in `main.py` liest es, `draft.py` nimmt es als `kind` entgegen), damit ein Abruf keine halbfertige Serie wegwirft und umgekehrt. **Serien**: nach dem Anlegen eines Artikels folgt der Ergebniskarte sofort eine neue Eingabekarte mit denselben Werten (`/create-item` liefert beide Fragmente plus den Zähler `item_count` per `hx-swap-oob`), und beim erneuten Öffnen der Seite kommt sie ebenso zurück, solange keine Eingabekarte mehr übrig ist — nur das Abweichende muss geändert werden. „Felder leeren“ (`POST /manual/reset`) beginnt eine neue Serie; der Knopf ist bewusst **kein** Submit-Button (als erster im Formular wäre er der Default-Button, und Enter in einem einzeiligen Feld hätte die Serie gelöscht statt den Artikel angelegt) — `resetManual()` schickt das Formular selbst, wartet dabei auf das bereits laufende Auto-Save und bestellt jedes weitere ab, sonst schreibt die Seite beim Verlassen genau das zurück, was gerade gelöscht wurde (v0.15.1). Gezeigt werden höchstens `MANUAL_RESULTS_SHOWN` fertige Karten — jede fordert ihr Etikettenbild neu an, eine offene Serie würde die Seite sonst mit jedem Artikel langsamer machen.
3. `POST /create` — Items anlegen (POST + PUT Kaufinfos), Result-Seite: Asset-IDs, Label-Vorschau, Druck (Kopienzahl, Asset-ID an/aus, 2 oder 3 Codes pro Reihe — v0.13.1; vorher gab die Karte nur wieder, womit gedruckt worden war). **Eine Statuszeile je Karte (v0.12.1)**: der automatische Druck beim Anlegen schreibt in dieselbe Box, die der Nachdruck-Knopf per htmx tauscht (`_print_status.html`, Ziel `#print-status-{idx}`) — jeder Versuch ersetzt die Antwort des vorherigen. Das Ergebnis landet zusätzlich im Entwurf (`draft.update_print_result`, über die Asset-ID gegen einen veralteten Karten-Index abgesichert), damit ein geglückter Nachdruck einen Reload von `/edit` übersteht — samt Layout (Codes pro Reihe, Asset-ID an/aus), das die Karte vor dem Nachdruck ändern kann (v0.13.2). Auch „kein Drucker" ist ein Versuch und wird so festgehalten, sonst stünde nach einem Reload wieder das „gedruckt" eines früheren Etiketts da (v0.13.3). Wie viele Codes wirklich gezeichnet werden, sagt `labels.clamp_qr_per_row()` — wer entscheidet, ob die Asset-ID noch Platz hat (Vorschau, `/print`, `_qr_per_row_off()`, Testdruck), fragt zuerst dort (v0.13.3). Die Asset-ID-Antwort wird dabei vom Etikett getrennt gehalten (`want_asset_id` neben `show_asset_id`): die Checkbox ist bei drei pro Reihe deaktiviert und wird deshalb gar nicht mitgeschickt — sie reist im Hidden-Feld `item-{i}-showid-want` bzw. als `data-was-checked` auf der Box, damit das Abwählen von „drei nebeneinander“ sie auch nach einem Reload zurückgibt (v0.13.4). Jede Result-Karte trägt dabei ihren Formular-Index (`card_idx`), nicht ihre Position in der Liste: `/create` überspringt Karten, die schon per eigenem Knopf angelegt oder in der UI entfernt wurden, und ein gezählter Index zeigte den Nachdruck danach auf den Entwurf einer anderen Karte — die Asset-ID-Sicherung verwirft ihn dort, und nach einem Reload stand wieder der Status vom Anlegen da (v0.13.5).
...
4. Fehlerpfade: SessionExpired → Link zu Settings; Homebox/Print-Agent offline → klare Meldung, Formulardaten bleiben erhalten. Druckfehler werden beim Anzeigen eingeordnet, nicht beim Speichern (`printer.error_key` → Locale-Key, `_print_error_parts` baut Satz + technisches Detail): fehlende Gerätedatei = „Drucker nicht angeschlossen", keine Antwort = „Druckdienst nicht erreichbar", alles andere bleibt wörtlich. So bekommen auch alte gespeicherte Fehler und ein Sprachwechsel den Satz.
...
- Browser-Tests (v0.14.1, `server/tests/test_browser.py`): die App läuft im Testprozess auf einem freien Port, Playwright/Chromium fährt die Edit-Seite an und misst, was gerendertes HTML nicht verrät — Breite eines Bedienelements, Zeile eines Feldes, seitliches Scrollen bei 400 px. Ohne installierten Browser überspringen sie sich (`playwright install chromium`), der Rest der Suite bleibt browserfrei. Anlass: das eingequetschte Shop-Dropdown fiel keinem Test auf.
