...
3. `POST /create` — Items anlegen (POST + PUT Kaufinfos), Result-Seite: Asset-IDs, Label-Vorschau, Druck (Kopienzahl, Asset-ID an/aus, 2 oder 3 Codes pro Reihe — v0.13.1; vorher gab die Karte nur wieder, womit gedruckt worden war). **Eine Statuszeile je Karte (v0.12.1)**: der automatische Druck beim Anlegen schreibt in dieselbe Box, die der Nachdruck-Knopf per htmx tauscht (`_print_status.html`, Ziel `#print-status-{idx}`) — jeder Versuch ersetzt die Antwort des vorherigen. Das Ergebnis landet zusätzlich im Entwurf (`draft.update_print_result`, über die Asset-ID gegen einen veralteten Karten-Index abgesichert), damit ein geglückter Nachdruck einen Reload von `/edit` übersteht — samt Layout (Codes pro Reihe, Asset-ID an/aus), das die Karte vor dem Nachdruck ändern kann (v0.13.2).
